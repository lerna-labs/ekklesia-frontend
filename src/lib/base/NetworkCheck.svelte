...
<Popover.Root>
	<Popover.Trigger class={children ? 'children' : 'w-full'}>
		{#if children}
			{@render children()}
		{:else}
			<div class="w-full bg-orange-600 p-2 text-center text-xs font-semibold uppercase text-white">
	...
	<Popover.Content class="w-[450px] text-xs">
		...
		<div class="grid grid-cols-2 gap-x-4 gap-y-1">
			...
			<div><span class="font-semibold">Node Version:</span> {serverStatus.server.nodeVersion}</div>
		</div>
