					...
					tooltip: {
						...
						callbacks: {
							title: (items) => {
								...
								const b = buckets[i];
								if (!b) return '';
								const mn = Number.isInteger(b.bucketMin)
									? b.bucketMin
									: b.bucketMin.toFixed(1);
								const mx = Number.isInteger(b.bucketMax)
									? b.bucketMax
									: b.bucketMax.toFixed(1);
								return `${mn} to ${mx}`;
							},
							label: (ctx) => {
	...
	function buildMedianLinePlugin() {
		...
		return {
			...
			afterDatasetsDraw(c) {
				...
				const label = 'Median ' + (Number.isInteger(medianMark) ? medianMark : medianMark.toFixed(1));
