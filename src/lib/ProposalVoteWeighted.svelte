<script>
	...
	// weighted is a SelectionEntry[] (`[{option, value}, ...]`).
	...
	const entries = $derived(
		pendingEntries ?? (hasSelection ? draft.selection : [])
	);
		...
		>
			...
			<div>
				...
				<p class="mt-0.5">
					...
					{:else}
						Allocate exactly {voterBudget} across the options
						({remaining} remaining) — partial allocations will be rejected on submission.
						Abstain if you'd prefer not to allocate at all.
					{/if}
				</p>
			</div>
		</div>
