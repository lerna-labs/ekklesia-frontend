<script>
  ...
  const maxSelections = $derived(Number(proposal?.maxSelections) || voterBudget || options.length);
  ...
  const selectionsRemaining = $derived(isBudget ? 0 : Math.max(0, maxSelections - selected.length));
