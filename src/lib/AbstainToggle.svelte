	...
	>
		...
		<div class="flex-1">
			...
			<p class="mt-0.5 text-xs">
				Your voting power will not be counted on this question. Your abstention is
				recorded separately from any per-option tally.
			</p>
		</div>
	</div>
{/if}
	...
	<div class="mt-3 flex flex-wrap items-center gap-2 border-t pt-3">
		...
		{#if showAbstainButton}
			<p class="basis-full text-[11px] text-muted-foreground">
				Abstaining records no opinion and counts toward a separate abstention tally —
				distinct from clearing, which leaves the question out of your submission entirely.
			</p>
		{/if}
	</div>
{/if}
