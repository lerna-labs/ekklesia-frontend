  ...
  {#if hasSelection && !isComplete && !isAbstaining}
    ...
    >
      ...
      <div>
        ...
        <p class="mt-0.5">
          Rank {remainingToRank} more option{remainingToRank === 1 ? '' : 's'} before submitting. Partial
          rankings are saved as drafts but will be rejected on submission — abstain if you don't want
          to rank this question.
        </p>
      </div>
    </div>
  {/if}
