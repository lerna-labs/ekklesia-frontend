...
{#if state === 'certified'}
  ...
  >
    ...
    <div class="flex-1">
      ...
      <p class="mt-1">
        These tallies have been re-weighted against the voting authority's audited snapshot{certification?.certifiedAt
          ? ' on ' + convertTimestamp(certification.certifiedAt)
          : ''}{certification?.snapshotEpoch != null
          ? ` (epoch ${certification.snapshotEpoch})`
          : ''}. Counts and voting power reflect who was eligible, and how much weight they carried,
        at the audited snapshot — not at the moment each vote was cast.
      </p>
...
{:else if state === 'narrative'}
  ...
  >
    ...
    <div class="flex-1">
      ...
      <p class="mt-1">
        The voting authority has published a narrative write-up of this ballot without re-weighting
        the tally. The numbers below are the Hydra-finalized record — what voters answered, weighted
        by the voting power snapshotted when each vote was validated. Read the authority's narrative
        for context on winners and losers.
      </p>
      {#if result?.updatedAt}
        <p class="mt-1 text-xs opacity-75">
          Tally last updated {convertTimestamp(result.updatedAt)}
...
{:else if state === 'hydra-provisional'}
  ...
  >
    ...
    <div class="flex-1">
      ...
      <p class="mt-1">
        Voting has closed and the tally has been committed on-chain, but the voting authority hasn't
        published an audited snapshot yet. These numbers are the Hydra-final record, weighted by
        voting power at the time each vote was cast — certification may adjust them once the
        authority's snapshot lands.
      </p>
...
{:else if state === 'voting'}
  ...
  >
    ...
    <div class="flex-1">
      ...
      <p class="mt-1">
        Voting is still open. These numbers are a running tally and will keep moving as new votes
        come in and voting power shifts. The backend recomputes the tally every few minutes.
      </p>
      {#if result?.updatedAt}
        <p class="mt-1 text-xs opacity-75">
