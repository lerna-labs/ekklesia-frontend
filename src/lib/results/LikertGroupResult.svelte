					...
					<table class="w-full border-collapse text-xs">
						<thead>
							...
							>
								...
								<th class="py-2 px-2 text-right font-semibold">Mean</th>
								<th class="py-2 px-2 text-right font-semibold">Median</th>
								<th class="py-2 px-2 text-right font-semibold">Mode</th>
								<th class="py-2 px-2 text-right font-semibold">Std dev</th>
								<th class="py-2 px-2 text-right font-semibold">n</th>
							</tr>
						</thead>
						<tbody>
							{#each options as opt}
								...
								<tr class="border-t border-slate-100">
									...
									<td class="py-2 px-2 text-right font-mono tabular-nums">{fmt(s?.mean, 2)}</td>
									<td class="py-2 px-2 text-right font-mono tabular-nums">{fmt(s?.median, 1)}</td>
									<td class="py-2 px-2 text-right font-mono tabular-nums">{fmtMode(s?.mode)}</td>
									<td class="py-2 px-2 text-right font-mono tabular-nums">{fmt(s?.stdDev, 2)}</td>
									<td class="py-2 px-2 text-right font-mono tabular-nums">{s?.count ?? 0}</td>
								</tr>
							{/each}
						</tbody>
					</table>
				...
				<details class="mt-3 rounded-md border border-slate-200 bg-slate-50/60">
					...
					<div class="border-t border-slate-200 p-3">
							...
							>
								<thead>
									...
									>
										...
										{#each grades as grade, i}
											<th
												class="py-2 px-2 text-center font-semibold"
												colspan="2"
											>
									...
									>
										{#each grades as _grade}
											<th
												class="py-1 px-2 text-right font-normal"
										...
										<tr class="border-t border-slate-100">
											...
											{#each grades as _grade, i}
												...
												{@const winText = wins
													...
													: ''}
												<td
													class="py-2 px-2 text-right font-mono tabular-nums {winClass}"
													title={wins
														? tied
												...
												<td
													class="py-2 px-2 text-right font-mono tabular-nums text-muted-foreground {winClass}"
												>
						...
						<p class="mt-2 text-[10px] italic text-muted-foreground">
							Each grade has two columns: the raw {dimension === 'power' ? 'voting power' : 'count'} at that grade,
							and the <em>above</em> / <em>below</em> percentages — support / opposition
							if the grade is treated as the candidate MJ median. Columns are
							discrete <code>td</code>s for clean spreadsheet copy.
							A
							<span class="rounded bg-emerald-50 px-1 py-[1px] text-emerald-700">
								green cell
							</span>
							marks a unique MJ tiebreaker winner for that candidate median — higher
							support, or lower opposition when support ties. An
							<span class="rounded bg-amber-50 px-1 py-[1px] text-amber-700">
								amber cell
							</span>
							marks options that tied on both support AND opposition — apply the
							ballot's remaining tiebreaker rules to resolve. Toggle "By power" above
							to run the same tabulation against weighted voting power.
						</p>
					</div>
				</details>

				<p class="mt-2 text-[10px] italic text-muted-foreground">
					The voting authority publishes the canonical interpretation of these ratings —
					this card presents the raw per-option distribution and descriptive statistics
					only, in ballot-declared order.
				</p>
			</div>
		{/if}
	{/snippet}
