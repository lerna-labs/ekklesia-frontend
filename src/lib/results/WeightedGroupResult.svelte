			...
			<div class="flex h-full flex-col">
				...
				{#if abstainCount > 0}
					...
					<div class="mb-3">
						...
						>
							{#if allocVal > 0}
								<div class="h-full" style="width: {allocPct}%; background-color: {accentColor};"></div>
							{/if}
							{#if abstainVal > 0}
		...
		{#if hasWeighted}
			<div class="border-t pt-3">
							...
							{#each sortedOptions as opt}
								...
								<tr class="border-t border-slate-100">
									...
									{#if dimension === 'count'}
										...
										{#if hasZeroAllocations}
											<td class="py-2 px-2 text-right font-mono tabular-nums text-muted-foreground">
				...
				<p class="mt-2 text-[10px] italic text-muted-foreground">
					<strong>Mean</strong> is per answering ballot — voters who allocated 0 to an option
					count in the denominator as implicit zeros. The parenthesized percent shows the
					mean as a share of the voter budget.
					<strong>Std dev</strong> measures how much voters disagreed on this option's
					allocation — low values indicate consensus, high values indicate divided
					opinion. Shares the same per-answering-ballot denominator, so implicit zeros
					from voters who skipped this option widen the spread.
					{#if hasZeroAllocations}
						<strong>Contrib. mean</strong> is the contributors-only average
						(<code>total / contributors</code>) — useful to read alongside the Hydra
						mean when some voters skipped this option entirely.
					{/if}
