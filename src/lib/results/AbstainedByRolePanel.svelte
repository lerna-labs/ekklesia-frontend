...
{#if entries.length > 0}
	...
	>
		...
		<p class="mb-3 text-xs text-muted-foreground">
			Voters who explicitly recorded no opinion on this question. Their voting power is
			not counted in the per-option results above — they're tallied here instead.
		</p>
		...
		{#if entries.length > 1}
			<div class="mt-2 text-[11px] font-mono tabular-nums text-muted-foreground">
