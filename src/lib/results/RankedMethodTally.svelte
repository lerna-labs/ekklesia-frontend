<script>
  ...
  const pairwiseOptions = $derived(Array.isArray(pairwise?.options) ? pairwise.options : []);
  const pairwiseMatrix = $derived(Array.isArray(pairwise?.matrix) ? pairwise.matrix : []);
  const pairwiseMax = $derived(
    pairwiseMatrix.reduce((m, row) => row.reduce((mm, v) => (v > mm ? v : mm), m), 0),
  );
</script>
  ...
  {#snippet details()}
    {#if hasFirstPref}
      ...
      >
        ...
        <p>
          Hydra publishes the raw rank data without naming a winner — IRV / STV / Condorcet all
          require tie-break rules that aren't universally canonical. The voting authority may
          publish their chosen interpretation separately.
        </p>
      </div>
              ...
              <thead>
                <tr>
                  ...
                  {#each pairwiseOptions as colId}
                    <th
                      class="px-1 py-1 text-center font-normal text-muted-foreground"
