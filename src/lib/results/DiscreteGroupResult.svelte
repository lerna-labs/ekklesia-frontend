<script>
  ...
  const optionCosts = $derived((proposal?.voteOptions ?? []).map((o) => Number(o.cost) || 0));
      ...
      <table class="w-full border-collapse text-xs">
          ...
          >
            ...
            {#if isBudget}
              <th
                ...
                title={capacityUnits ? `Cost per option, in ${capacityUnits}` : 'Cost per option'}
              >
                Cost
              </th>
            {/if}
        ...
        <tbody>
          {#each tableRows as row}
            ...
            {@const powerPctRow = totalGroupPower ? (row.votingPower / totalGroupPower) * 100 : 0}
              ...
              {#if isBudget}
                <td
                  class="py-2 pl-2 text-right font-mono tabular-nums {cum?.overBudget
                    ...
                    : ''}"
                  title={cum?.overBudget ? 'Cumulative cost exceeds the voter budget' : null}
                >
                  {#if cum?.value == null}
                    <span class="text-muted-foreground">—</span>
                  {:else}
      ...
      {#if isBudget}
        <p class="mt-2 text-[10px] italic text-muted-foreground">
          The Cumulative column assumes a top-{dimension === 'power'
            ? 'voting-power'
            : 'votes'}-first selection rule — amber cells are where that rule would exceed the voter
          budget.{hasWeight
            ? ' Flip the Sort-by toggle above to see how the cutoff moves under the other dimension.'
            : ''} The voting authority may apply a different rule (optimal knapsack, weighted threshold,
          etc.), so treat this cutoff as one possible reading rather than the authoritative outcome.
        </p>
      {:else if isPreference}
        <p class="mt-2 text-[10px] italic text-muted-foreground">
          Options are ordered by {dimension === 'power' ? 'voting power' : 'voter count'}.{hasWeight
            ...
            : ''} The voting authority may apply rules this card can't represent (acceptance thresholds,
          minimum vote share, quorum-per-option, etc.), so treat this ordering as descriptive rather than
          authoritative.
        </p>
      {/if}
    {/if}
