...
{#if checked && activePackage && activePackage.status === 'awaiting-signatures'}
  <div
    class="mb-4 mt-4 rounded-md border border-indigo-300 bg-indigo-50 p-3 text-sm text-indigo-900"
  >
    ...
    {#if activePackage.multisig}
      <p class="mb-2 text-xs">
        ...
        {activePackage.multisig.outstandingKeys?.length ?? 0} signer{(activePackage.multisig
          .outstandingKeys?.length ?? 0) === 1
          ? ''
          : 's'}
