		...
		<div class="flex-1">
			...
			<p class="mt-1">
				{#if hasTypeMismatch}
					...
					<span class="font-semibold">{credentialLabel(voterCredential)}</span>, so vote
					options on each proposal are read-only — reconnect with an eligible wallet
					identity to cast a vote.
				{:else}
					You're logged in, but your
					<span class="font-semibold">{credentialLabel(voterCredential)}</span>
					identity does not meet this ballot's eligibility requirements. Vote options on
					each proposal are read-only; contact support if you believe this is wrong.
				{/if}
			</p>
		</div>
	</div>
