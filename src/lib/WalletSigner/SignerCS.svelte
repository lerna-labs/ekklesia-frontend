<script>
  ...
  import {
    ...
    signData,
    submitSignature,
  } from "$lib/WalletSigner/WalletSigner.js";
  ...
  async function requestPayload() {
    loading = true;
    let url =
      mode === "login" ? "/session" : "/dashboard/" + ballot._id + "/checkout";
    if (mode !== "login" && multiSig) {
      url += "/multisig";
    }
    if (tx) {
      url += "/" + tx;
...
{#if !receivedPayload}
  <Input type="text" {placeholder} class="w-full" bind:value={signerAddress} />
...
{#if receivedPayload}
  ...
  <Textarea
    ...
    class="mt-2 w-full text-xs max-h-[200px] min-h-[80px] resize-none overflow-y-auto"
  ...
  <Button
    ...
    disabled={!signerAddress || !signatureJSON}
  >
    {#if !signerAddress}
      Select how to sign
