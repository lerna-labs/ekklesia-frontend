<script>
  ...
  const enabled = $derived(
    ballot?.source === 'hydra' && ballot?.hydra && ballot?.status !== 'upcoming',
  );
  ...
  const ballotCid = $derived(
    h.ballotCid ||
      (ekklesia.ballotIpfsCid && ekklesia.ballotIpfsCid !== 'self' ? ekklesia.ballotIpfsCid : null),
  );
  ...
  onMount(async () => {
    ...
    try {
      ...
      console.warn('Close-receipt lookup failed:', err?.body?.message || err?.message || err);
    }
  });
</script>
        ...
        <div class="flex items-start justify-between gap-3">
          <div class="min-w-0 flex-1">
            ...
            <Card.Description class="text-xs">
              ...
              verifiable against on-chain artifacts.
              {#if headStatus}
                <span
                  class="ml-1 inline-block rounded bg-muted px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide"
                >
                  {headStatus}
                </span>
              {/if}
      ...
      {#if merkleRoot}
        <div>
          ...
          <p class="mt-0.5 text-[11px] text-muted-foreground">
            Cryptographic digest of the ballot definition itself — proposals, options, voting rules.
            Anchored on-chain via the prepare transaction so anyone can verify the ballot they're
            voting on matches what's committed on-chain (no silent parameter changes mid-vote).
          </p>
