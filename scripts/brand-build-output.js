...
if (deployment !== '_default') {
	...
	if (fs.existsSync(brandDir)) {
		...
		for (const filename of BRAND_ASSETS) {
			...
			console.log(`[brand] copied ${path.relative(process.cwd(), src)} → ${path.relative(process.cwd(), dst)}`);
			copied += 1;
		}
		if (copied === 0) {
			console.log(`[brand] no favicon overrides found in ${path.relative(process.cwd(), brandDir)}`);
		}
	}
}
